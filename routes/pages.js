...
router.get('/',(req,res)=>{
    db.query('SELECT * FROM post ',async (error,results)=>{
     console.log(results);
    ...
    if(category=="select"){
        ...
        db.query('SELECT * FROM post WHERE category=? AND userID!=?',[category , res.locals.user.id],async (error,results)=>{
            if(error){
               ...
               db.query('SELECT * FROM folowers WHERE UserID=?',[res.locals.user.id],async (error,results1)=>{
                if(error){
                    console.log(error);
                }
                else{
                ...
                if(error){
                    ...
                    db.query('SELECT * FROM post_comment WHERE postID=?',[req.query.post],async (error,results2)=>{
                        if(error){
                            ...
                            db.query('SELECT * FROM folowers WHERE postID=?',[req.query.post],async (error,result)=>{
                                console.log(result);
